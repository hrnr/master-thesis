...
The presented reciprocal matching algorithm (Algorithm~\ref{alg:cross-match}) using a more strict non-probabilistic approach is able to avoid the upside down match. However, inliers are clustered in one particular area of the map (Figure~\ref{}), which leads to a visible angular error. Other parts of the common area proved unable to provide stable matches, especially the circular area seems to contain only unstable features and more mapping errors.
