...
The datasets used for evaluation were recorded in two different spaces ``Rotunda'', a half-circle computer laboratory with adjacent rooms (Figure~), and ``Refectory'', containing a baroque refectory and lecture rooms at the first floor of the faculty building (Figure~). In each of the environments two maps were recorded.

The maps were generated by RTAB-Map \gls{SLAM} as in the previous datasets. I used extended Kalman filter to fuse odometry data from the gyroscope, wheel encoders and the visual odometry provided by the native RTAB-Map approach. The filtering was using \gls{2D} assumption as the robot operates in the plane. Together, those data provided a reliable odometry source for \gls{SLAM}. The resulting maps were voxelized to the resolution of 0.05 meters per voxel.
