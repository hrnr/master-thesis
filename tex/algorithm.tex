...
To overcome the issue \gls{SAC-IA} algorithm has been proposed in~\cite{rusu2009fpfh}. The algorithm is shown in figure~\ref{alg:sac-ia}.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Function{matchKReciprocal}{$D_1, D_2$}
            \ForAll{$d_i \in D_1$}
                \State find $k$-nearest neighbours of $d_i$ in $D_2$
                \State $m_i \gets$ random sample from $M$
            \EndFor
        \EndFunction
    \end{algorithmic}
